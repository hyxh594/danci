(() => {
  const STORAGE_KEY = "fuci-cet6-state-v1";
  const now = () => Date.now();
  const DAY = 86400000;
  ...
  const save = () => { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); clearTimeout(saveTimer); $("#save-indicator").textContent = "已保存"; };
  ...
  function renderCard() {
    ...
    if (!currentWord) { $("#review-title").textContent = "今天完成了"; $("#word-main").textContent = "太棒了"; $("#word-phonetic").textContent = "暂时没有待复习的词"; $("#word-answer").textContent = "去词库添加新词，或者明天再来看看。"; $("#word-answer").classList.add("visible"); $("#reveal-btn").disabled = true; $("#forgot-btn").disabled = true; $("#remember-btn").disabled = true; $("#skip-btn").disabled = true; return; }
    const p = progressFor(currentWord.id);
    $("#review-title").textContent = p.status === "new" ? "认识新词" : "巩固记忆";
    $("#word-main").textContent = currentWord.word;
    $("#word-phonetic").textContent = "CET-6 核心词 · 先回忆，再揭晓";
  ...
  function review(result) {
    ...
    state.progress[currentWord.id] = p; state.history.unshift({ wordId: currentWord.id, result, reviewedAt: now() }); state.history = state.history.slice(0, 1000); save(); updateStats(); toast(result === "remembered" ? `已记住 · ${p.interval} 天后复习` : "已加入稍后复习");
    queuePosition += 1; currentWord = queue[queuePosition] || null; revealed = false; renderCard();
  }
  ...
  function updateStats() {
    const all = state.words; $("#stat-total").textContent = all.length; $("#stat-learning").textContent = all.filter((w) => progressFor(w.id).status === "learning").length; $("#stat-mastered").textContent = all.filter((w) => progressFor(w.id).status === "mastered").length; $("#stat-due").textContent = dueWords().length; renderTable();
  }
  ...
  function addWord(word, meaning) { const clean = word.trim().toLowerCase(); if (!clean || state.words.some((w) => w.word === clean)) return false; state.words.unshift({ id: `custom-${clean}-${Date.now()}`, word: clean, meaning: meaning.trim(), level: "自定义", source: "用户添加", createdAt: now() }); save(); updateStats(); return true; }
  function importText(text, fileName) { let imported = []; try { if (fileName.endsWith(".json")) { const parsed = JSON.parse(text); imported = Array.isArray(parsed) ? parsed : parsed.words || []; } else imported = text.split(/\r?\n/).map((line) => { const parts = line.split(/[,\t|]/); return { word: parts[0], meaning: parts.slice(1).join("，") }; }); } catch { toast("文件格式无法识别"); return; } let count = 0; imported.forEach((item) => { const word = item.word || item.term || item.单词; const meaning = item.meaning || item.translation || item.释义 || ""; if (word && addWord(String(word), String(meaning))) count += 1; }); toast(`已导入 ${count} 个新词`); updateStats(); }
  ...
  function bind() {
    ...
    $("#reveal-btn").addEventListener("click", reveal); $("#remember-btn").addEventListener("click", () => review("remembered")); $("#forgot-btn").addEventListener("click", () => review("forgotten")); $("#skip-btn").addEventListener("click", skip);
    document.addEventListener("keydown", (event) => { if (event.target.matches("input")) return; if (event.code === "Space") { event.preventDefault(); reveal(); } if (event.key === "1") review("forgotten"); if (event.key === "2") review("remembered"); });
    $("#search-input").addEventListener("input", renderTable); $("#export-btn").addEventListener("click", exportBackup); $("#import-btn").addEventListener("click", () => $("#import-file").click()); $("#import-file").addEventListener("change", (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => importText(String(reader.result), file.name.toLowerCase()); reader.readAsText(file); e.target.value = ""; });
  ...
  document.querySelector(".app-shell").style.opacity = state.settings.opacity; bind(); applySuperMode(state.settings.superMode, false); startSession(); updateStats(); fitDesktopWindow(); if (state.settings.reminder && dueWords().length) setTimeout(() => toast(`今天有 ${dueWords().length} 个词待复习`), 500);
})();
