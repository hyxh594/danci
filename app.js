(() => {
  ...
  function fitDesktopWindow({ compact = false } = {}) { if (!state.settings.superMode || !window.fuciDesktop?.fitToContent) return; clearTimeout(fitTimer); fitTimer = setTimeout(() => { const card = $("#word-card"); if (!card) return; const rect = card.getBoundingClientRect(); const width = Math.max(220, Math.ceil(rect.width)); const neededHeight = Math.max(176, Math.ceil(card.scrollHeight || rect.height)); const currentHeight = Math.ceil(window.innerHeight || 0); const autoCompact = compact && !state.settings.superSizeManual; const staleHeight = hasStoredSuperSize() && currentHeight > neededHeight + 4; if (hasStoredSuperSize() && !staleHeight && neededHeight <= currentHeight + 4 && !autoCompact) return; if (autoCompact || staleHeight) { state.settings.superSize = { width, height: neededHeight }; if (autoCompact) { autoFitTarget = { width, height: neededHeight }; clearTimeout(autoFitClearTimer); autoFitClearTimer = setTimeout(() => { autoFitTarget = null; }, 1000); } } window.fuciDesktop.fitToContent(width, neededHeight); if ((autoCompact || staleHeight) && !appStarting) save({ backup: false }); }, 90); }
